...
class GoogleCloudStorageConfig:
    ...
    project_name: str = attr.ib(default='morph-and-split-engine')

    # Service Account
    service_account_name: str = attr.ib(default='morph-and-split-engine-sa')
    service_account_key_file_name: str = attr.ib(
        default='morph-and-split-engine-key.json')
    service_account_key_file_path: Optional[str] = attr.ib(
        init=False, default=None)
    service_account_email: str = attr.ib(init=False, default=None)

    # Details for impersonated credentials.source
    target_scopes: list = attr.ib(
        factory=lambda: ['https://www.googleapis.com/auth/cloud-platform'])
    ...
    responseHeader: List[str] = attr.ib(
        factory=lambda: ["Content-Type", "x-goog-content-resumable"])
    ...
    resized_train_images_dir: str = attr.ib(
        default='resized_augmented/train/images')
    resized_train_masks_dir: str = attr.ib(
        default='resized_augmented/train/masks')
    resized_val_images_dir: str = attr.ib(
        default='resized_augmented/val/images')
    resized_val_masks_dir: str = attr.ib(default='resized_augmented/val/masks')
    resized_test_images_dir: str = attr.ib(
        default='resized_augmented/test/images')
    resized_test_masks_dir: str = attr.ib(
        default='resized_augmented/test/masks')
    augmented_dir: str = attr.ib(default='augmented/combined')

    def __attrs_post_init__(self):
        self.service_account_key_file_path = str(os.path.join(
            current_directory(), self.service_account_key_file_name))
